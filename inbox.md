...
- [Claude Fable 5 and Claude Mythos 5](https://www.anthropic.com/news/claude-fable-5-mythos-5)
  Anthropic, June 9 2026. First frontier-lab release positioning *scientific research* as a headline capability area, not an afterthought. Official claims about Mythos 5 (restricted access): "first model to consistently produce novel, compelling scientific hypotheses"; ~80% researcher preference for its molecular biology hypotheses over Opus-class baseline; one E. coli protein-mechanism hypothesis **corroborated by an independent lab** working on the same problem; ~10× speedup in drug-design workflows; week-long autonomous genomics study across 138 animal species with a custom ML model outperforming a Science-published baseline at 100× smaller. Fable 5 is the publicly available version. Strong on-beat — directly affects research practice per the vision-doc frontier-lab exception. Pair this with the AutoScientists/Tang-Yang governance-vs-capability tension (010) and the Leiden Declaration (this inbox) for a full "capability is here; now what" arc in edition 012.
