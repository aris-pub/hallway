...
- [Claude Fable 5 and Claude Mythos 5](https://www.anthropic.com/news/claude-fable-5-mythos-5)
  Anthropic, June 9 2026. Third frontier-lab research-positioned release in eight weeks (after GPT-Rosalind April 16 and Google's Gemini for Science May 19), but the first where a **flagship general-purpose model** carries scientific research as a top-line capability claim alongside coding/vision/knowledge work — the prior two were a domain-specialized sidecar and a tool suite respectively. Specific Mythos 5 claims: "first model to consistently produce novel, compelling scientific hypotheses"; ~80% researcher preference vs. Opus-class baseline on molecular biology; one E. coli protein-mechanism hypothesis **corroborated by an independent lab**; ~10× speedup in drug design; week-long autonomous genomics across 138 species with a custom ML model **beating a Science-published baseline at 100× smaller**. Fable 5 is the public version. The trajectory itself — three labs competing on research capability as a marketing axis in eight weeks — is the real signal. Pair with the Leiden Declaration (this inbox) and the Tang-Yang/AutoScientists governance threads (010) for a "capability is here; now what" arc in edition 012.
